...
from action.model.MotionAGFormer_action import MotionAGFormer_action
...
def load_backbone(args):
    if not (hasattr(args, "backbone")):
        args.backbone = 'MotionAGFormer_action'  # Default
        args.model_name = 'MotionAGFormer_action'
    if args.backbone == 'MotionAGFormer_action':
        act_mapper = {
            "gelu": nn.GELU,
            'relu': nn.ReLU,
            'symsum': SymSum
        ...
        if args.model_name == "MotionAGFormer_action":
            model_backbone = MotionAGFormer_action(n_layers=args.n_layers,
                                          dim_in=args.dim_in,
